...
try:
	...
	campaign = client.campaigns.list(count=5, since_send_time='{}T11:00:00.000Z'.format(last_send), before_send_time="{}T22:00:00.000Z".format(last_send), folder_id=os.environ['FOLDER_ID'], fields=["campaigns.id", "campaigns.recipients"])  
...
top_clicks = sorted(click_report['urls_clicked'], key=sortLinks, reverse=True)[:15]
